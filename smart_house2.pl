...
bloquear:- accion(abertura(X),bloqueada).
bloquear:- ubicacion(persona(X),habitacionX), accion(abertura(X),bloqueada). 

desbloquear:- accion(abertura(X),desbloqueada).
habilitarSeg:- verEstadoCasa(result), result = vacia, setAlarma(on).
desbloquear:- verEstadoCasa(result), result = vacia, setAlarma(on);
	      abrirComponenteCasa.	

setLucesDelPatio:- verEstadoCasa(result), result = noche, accion(luz,power).
clima:- verEstadoCasa(result), result = vacia. % accion(aire_acondicionado, subir_temp).
